...
# Attempt to detect public IPv4 and IPv6 and populate relay-ip/external-ip automatically
PUBLIC_IPV4="$(curl -fsS --max-time 5 https://api.ipify.org || true)"
PUBLIC_IPV6="$(curl -fsS --max-time 5 https://api6.ipify.org || true)"

if [[ -n "$PUBLIC_IPV4" ]]; then
  echo "relay-ip=$PUBLIC_IPV4" >> "$CONF_FILE"
  echo "external-ip=$PUBLIC_IPV4" >> "$CONF_FILE"
fi

if [[ -n "$PUBLIC_IPV6" ]]; then
  echo "relay-ip=$PUBLIC_IPV6" >> "$CONF_FILE"
  # coturn requires external-ip as IPv4 for some configs; include both if present
  echo "external-ip=$PUBLIC_IPV6" >> "$CONF_FILE"
fi
# Automatically generated by scripts/setup_coturn.sh
listening-port=3478
tls-listening-port=5349
